...
for init_number in initial_number:
    ...
    senders = df['Sender Address'].tolist()
    receivers = df['Receiver Address'].tolist()
    total = senders + receivers
    unique_addresses = list(set(total))
    G = nx.from_pandas_edgelist(df,'Sender Address', 'Receiver Address')
    number_of_nodes.append(G.number_of_nodes())
    print(len(unique_addresses), G.number_of_nodes())
    temp = list(G.degree())
    degree_list = []
    for element in temp:
