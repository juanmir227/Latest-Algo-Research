...
def my_distro(x, alfa, mu):
    mean_x = mean_k
    return x**(-alfa)*np.exp((-2*mu)/(mean_x*(1-alfa))*x**(1-alfa))

stretch_exp, pcov = curve_fit(my_distro, xdata = bins, ydata = values, p0= (0.5, 1), sigma = density_error)
plt.errorbar(bins,values,yerr=density_error, label = 'data')
y = my_distro(bins, stretch_exp[0], stretch_exp[1])
print(y)
print(bins)
