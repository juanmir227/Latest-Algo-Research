...
def smooth(given_list):
    for i, element in enumerate(given_list):
        if element == given_list[0] or element == given_list[-1] or element == given_list[1] or element == given_list[-2]:
            ...
            element = (given_list[i-2] + given_list[i-1] +  element + given_list [i+1] + given_list[i+2])/5
            given_list[i] = round(element,2)
    return given_list
...
with open(r"D:\Archivos de Programa\Carpetas\Coding\Algorand\Tesis\Tesis\periodo3_lists_csv\periodo3_smooth_hard_list",'wb') as f:
